...
// Keep sessions persistent enough for normal use, but not effectively permanent.
const FALLBACK_SESSION_TTL_SEC = 30 * 24 * 60 * 60;
...
const resolveSessionSecret = () => {
    ...
    if (sharedFallbackSecret) {
        ...
        if (!warnedAboutSessionSecretFallback) {
            ...
            console.warn(
                '[security] AUTH_SESSION_SECRET is not set. Falling back to a shared application secret. '
                + 'Set AUTH_SESSION_SECRET in environment for dedicated session signing.'
            );
        }
        return cachedSessionSecret;
    }
    ...
    if (!warnedAboutSessionSecretFallback) {
        warnedAboutSessionSecretFallback = true;
        console.warn(
            '[security] AUTH_SESSION_SECRET is not set. Using ephemeral in-memory secret. '
            + 'Set AUTH_SESSION_SECRET in environment for dedicated session signing.'
        );
    }
    return cachedSessionSecret;
...
const shouldUseSharedCookieDomain = (host, rootDomain) =>
    host === rootDomain || host.endsWith(`.${rootDomain}`);

const resolveCookieDomain = (req) => {
    const configuredDomain = String(pickFirstEnv('AUTH_COOKIE_DOMAIN') || '').trim().toLowerCase();
    if (configuredDomain) {
    ...
    const sharedDomains = ['mahean.com', 'maheanahmed.com'];
    const matched = sharedDomains.find((rootDomain) => shouldUseSharedCookieDomain(host, rootDomain));
    if (!matched) return '';
    return `.${matched}`;
};
...
export const readSessionClaimsFromRequest = (req) => {
    ...
    return {
        ...
        role: payload.role === 'admin'
            ? 'admin'
            : (payload.role === 'moderator' ? 'moderator' : 'reader'),

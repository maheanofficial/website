...
- `CPANEL_PORT` (optional, SSH mode only, usually `22`)
- `CPANEL_FTP_PORT` (optional, FTP mode only, default `21`)
- `CPANEL_USER` (example: `mahean`)
- `CPANEL_PASSWORD` (recommended on shared hosting; if set, workflow deploys over FTP and triggers restart without shell access)
- `CPANEL_SSH_KEY` (private key content, raw multiline; used when `CPANEL_PASSWORD` is not set)
...
- `CPANEL_NODE_VENV_ACTIVATE` (optional, SSH mode only, default: `/home/<CPANEL_USER>/nodevenv/<app_dir_name>/20/bin/activate`)

What the workflow does:

1. Build app in GitHub Actions (`npm ci`, `npm run build`).
2. If `CPANEL_PASSWORD` is set, uploads the prepared files directly over FTP to the app directory and touches `tmp/restart.txt`.
3. If `CPANEL_PASSWORD` is not set, falls back to SSH key deploy: uploads a bundle, extracts it in app dir, runs `npm install`, `npm run db:init`, optional `npm run db:fix-encoding`, then triggers restart.
...
Note: FTP mode does not run server-side `npm install`. If you change dependencies, run one manual SSH deploy (or a one-time manual `npm install` on the server) before relying on FTP-only auto deploy.
...
### Weekly DB backup (automatic)

This repo now includes `.github/workflows/weekly-backup.yml` for automatic MySQL backups every week (Sunday, 02:00 UTC).

Required repository secrets for backup job:

- Works out of the box with existing deploy secrets:
  - `CPANEL_HOST`
  - `CPANEL_USER`
  - `CPANEL_PASSWORD`
- Optional overrides (if MySQL credentials differ from cPanel credentials):
  - `MYSQL_HOST`
  - `MYSQL_PORT` (optional, defaults to `3306`)
  - `MYSQL_USER`
  - `MYSQL_PASSWORD`
  - `MYSQL_DATABASE` (optional, auto-detected if unambiguous)
  - `MYSQL_TABLE_PREFIX` (optional, defaults to `app_table`)
  - `MYSQL_USERS_TABLE` (optional, defaults to `app_users`)

What it does:

1. Runs `node scripts/create-mysql-backup.cjs`.
2. Exports users table + all data tables with configured prefix.
3. Stores a compressed backup artifact (`.json.gz`) + checksum (`.sha256`) in GitHub Actions artifacts.
4. Keeps artifacts for 90 days.

Manual run command (local/server shell with MySQL env set):

```bash
npm run db:backup:mysql
```

### Uptime alerting (automatic)

This repo now includes `.github/workflows/uptime-monitor.yml` for continuous health checks every 10 minutes.

Defaults:

- Check URL: `https://www.mahean.com/healthz`
- Healthy response: HTTP `200` and body `ok`

Optional repository secret:

- `UPTIME_CHECK_URL` (override default health URL)

Alert behavior:

1. If health check fails, opens a GitHub issue titled `[Uptime] <url> is DOWN` (if not already open).
2. While down, workflow run is marked failed.
3. When recovered, it comments on the issue and auto-closes it.

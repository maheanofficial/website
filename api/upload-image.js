...
export default async function handler(req, res) {
    ...
    const runtimeR2Bucket = req?.cfEnv?.UPLOADS_BUCKET;

    if (runtimeR2Bucket && typeof runtimeR2Bucket.put === 'function') {
        try {
            await runtimeR2Bucket.put(`uploads/${relativePath}`, bytes, {
                httpMetadata: {
                    contentType: dataUrl.mime,
                    cacheControl: 'public, max-age=31536000, immutable'
                },
                customMetadata: {
                    uploaderId: auth.actor.id,
                    folder
                }
            });
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Failed to save image to R2.';
    ...
    // Worker runtime does not provide writable filesystem.
    if (req?.cfEnv) {
        json(res, 503, { error: 'Image upload is unavailable: Cloudflare R2 bucket is not configured.' });
        return;
    }

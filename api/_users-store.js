...
import { dbConfigError, isD1Enabled, isMysqlEnabled } from './_db-config.js';
import { readUsersFromD1, writeUsersToD1 } from './_d1-users-store.js';
import { readUsersFromMysql, writeUsersToMysql } from './_mysql-users-store.js';

const moduleFileUrl = typeof import.meta?.url === 'string' ? import.meta.url : '';
const __filename = moduleFileUrl ? fileURLToPath(moduleFileUrl) : '';
...
let generatedPrimaryAdminPassword = '';
let warnedMissingPrimaryAdminPassword = false;
const getPrimaryAdminPassword = () => {
    if (CONFIGURED_PRIMARY_ADMIN_PASSWORD) {
        return CONFIGURED_PRIMARY_ADMIN_PASSWORD;
    }
    if (!warnedMissingPrimaryAdminPassword) {
        warnedMissingPrimaryAdminPassword = true;
        console.warn(
    ...
    if (!generatedPrimaryAdminPassword) {
        generatedPrimaryAdminPassword = randomBytes(24).toString('hex');
    }
    return generatedPrimaryAdminPassword;
};
...
const sanitizeUser = (user) => {
    ...
    return {
        ...
        photoURL: normalizeAvatarUrl(user.photoURL),
...
const ensurePrimaryAdmin = (users) => {
    ...
    const primaryAdmin = sanitizeUser({
        ...
        password: adminIndex >= 0
            ? (normalized[adminIndex].password || getPrimaryAdminPassword())
...
export const readUsers = async () => {
    ...
    if (isD1Enabled) {
        const sourceUsers = await readUsersFromD1();

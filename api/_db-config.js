...
const sanitizeBindingName = (value, fallback) => {
    const normalized = String(value || '')
        .trim()
...
const D1_BINDING_NAME = sanitizeBindingName(
    pickFirstEnv('D1_BINDING', 'CF_D1_BINDING'),
    'D1_DB'
);

const isD1Requested = DB_BACKEND === 'd1';
...
export const isMysqlEnabled =
    !isD1Requested
    && (isMysqlRequested || (!isJsonRequested && isMysqlConfigured));

export const isD1Enabled = isD1Requested;

export const dbBackend = isD1Enabled
    ? 'd1'
    : (isMysqlEnabled ? 'mysql' : 'json');
...
export const d1BindingName = D1_BINDING_NAME;

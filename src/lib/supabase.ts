import { buildServerAuthHeaders } from '../utils/serverAuth';
...
const buildSession = (
    user: SupabaseUser,
    payload?: { sessionExpiresAt?: unknown }
): SessionLike => {
...
const normalizeServerRole = (value: unknown) => {
...
const mapServerUserToSupabaseUser = (user: unknown): SupabaseUser => {
    ...
    return {
        ...
        app_metadata: {
            role: normalizeServerRole(candidate.role),

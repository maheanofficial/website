...
const getUsers = (): User[] => {
    ...
    try {
        ...
        console.warn('Failed to parse local user storage; clearing corrupted state.', error);
        localStorage.removeItem(STORAGE_KEY);
    }

    let users = Array.isArray(parsed) ? parsed.map((user) => normalizeUser(user as User)) : [];
    let didMutate = false;
    const filteredUsers = users.filter((user) => !isBlockedUser(user) && !isLegacySeedUser(user));
...
export const registerUser = (email: string, password: string, displayName?: string): { success: boolean; message: string; user?: User } => {
    ...
    const newUser: User = {
        ...
        password,
        role: 'reader', // Default role for signup

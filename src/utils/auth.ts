...
import { sanitizeAuthNextPath, storeAuthRedirectIntent } from './authRedirect';
import {
    loginUser,
    registerUser,
...
const DEFAULT_SELF_SERVICE_ROLE: LocalUser['role'] = 'reader';
...
const ALLOW_LOCAL_AUTH_FALLBACK_BY_ENV = String(import.meta.env.VITE_ALLOW_LOCAL_AUTH_FALLBACK || '')
    .trim()
    .toLowerCase() === 'true';
...
const isGoogleAvatarUrl = (value?: string) => {
...
const normalizeAvatarUrl = (value?: string) => {
    ...
    if (!isGoogleAvatarUrl(raw)) return raw;

    const sizeSuffixPattern = /(?:=|\/)s\d+(?:-c)?(?:-[a-z0-9]+)*$/i;
    if (sizeSuffixPattern.test(raw)) {
        return raw.replace(sizeSuffixPattern, '=s512-c');
    }
...
const mapSupabaseUser = (user: SupabaseUser): LocalUser => {
    ...
    const photoURL = normalizeAvatarUrl(
        pickMetadataString(metadata.avatar_url) || pickMetadataString(metadata.picture)
    );
...
export const signInWithGoogle = async (nextPath?: string) => {
    ...
    storeAuthRedirectIntent(sanitizeAuthNextPath(nextPath, '/profile'));
...
export const updateCurrentUserProfile = async (updates: {
    ...
    let lastServerErrorMessage = 'Profile update failed.';
    if (typeof window !== 'undefined') {
        try {
            ...
            if (response.ok && payload?.user) {
                const nextUser = upsertUser({
                    ...currentUser,
                    ...(payload.user as Partial<LocalUser>),
                    role: toRole(String((payload.user as Record<string, unknown>)?.role || currentUser.role))
                } as LocalUser);
                setCurrentUserSession(nextUser);
                emitAuthChange('USER_UPDATED', nextUser);
                return nextUser;
            }
            ...
            console.warn('Server profile update failed', error);
            if (error instanceof Error && error.message) {
                lastServerErrorMessage = error.message;
            }
        }
    ...
    const fallbackUser = getLocalCurrentUser();
    if (!fallbackUser) {
        throw new Error('Session expired. Please log in again.');
    }

    const updatedUser = updateUserProfile(fallbackUser.id, {
        displayName: updates.displayName,
        username: updates.username,
        photoURL: updates.photoURL
    });
    ...
    emitAuthChange('USER_UPDATED', updatedUser);
    return updatedUser;
};

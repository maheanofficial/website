...
const Stories = () => {
    const [stories, setStories] = useState<Story[]>(() => getCachedStories().slice(0, 6));
                    ...
                    {stories.map((story) => (
                        <article key={story.id} className="story-card">
                            <div className="story-content">
                                {story.category ? (
                                    <Link to={buildCategoryFilterPath(story.category)} className="story-category" style={{ textDecoration: 'none' }}>
                                        {story.category}
                                    </Link>
                                ) : null}

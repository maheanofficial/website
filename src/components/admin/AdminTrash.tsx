...
const AdminTrash = () => {
    ...
    const handleRestore = async (id: string) => {
        if (window.confirm('Restore this item?')) {
            ...
            if (item) {
                ...
                else if (item.type === 'tag') await restoreDeletedTag(item.data);
                else if (item.type === 'story_part') await restoreDeletedStoryPart(item.data);
                await loadTrash();
            }
        }
    };
    ...
    const getTypeLabel = (type: string) => {
        switch (type) {
            ...
            case 'tag': return 'Tag';
            case 'story_part': return 'Story Part';
            default: return type;
        }
    };

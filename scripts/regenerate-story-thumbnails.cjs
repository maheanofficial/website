...
const STORY_DATA_PATH = path.join(ROOT_DIR, 'data', 'table-stories.json');
const FONT_PATH = path.join(ROOT_DIR, 'public', 'HindSiliguri-Ch41_erl.ttf');
...
const generateCover = async (title, author) => {
  ...
  // === BACKGROUND: solid black/dark gray ===
  ...
  try {
    if (fs.existsSync(LOGO_PATH)) {
      ...
      const logoX = canvasWidth - targetWidth - 20;
      const logoY = 20;
  ...
  for (let size = 56; size >= 36; size -= 4) {
  ...
  // === AUTHOR NAME ===
  const authorFontSize = Math.max(20, Math.round(titleFontSize * 0.45));
